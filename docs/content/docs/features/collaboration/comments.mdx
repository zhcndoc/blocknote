...
- 创建一个 `CommentsExtension` 实例，并将其传递给编辑器的 `extensions` 选项。
- 将 `resolveUsers` 传递给你的 `CommentsExtension` 实例，以便它可以检索和显示用户信息（姓名和头像）。
- 为你的 `CommentsExtension` 实例提供一个 `ThreadStore`，以便它可以存储和检索评论线程。
- 启用实时协作（参见 [实时协作](/docs/features/collaboration)）
- 可选地为评论和评论编辑器提供一个 schema。如果未定义，它们将使用 [默认评论编辑器 schema](https://github.com/TypeCellOS/BlockNote/blob/main/packages/react/src/components/Comments/defaultCommentEditorSchema.ts)。更多关于自定义 schema 的信息请参见 [这里](/docs/features/custom-schemas)。
