...
如果你的自定义内联内容应包含 [`StyledText`](/docs/foundations/document-structure#inline-content-objects)，则设置为 `styled`；如果不包含，则为 `none`。
...
`draggable:` 指定该内联内容是否可以在编辑器内拖动。如果设置为 `true`，该内联内容将可拖动。若未指定，默认值为 `false`。如果为 `true`，你应当将 `data-drag-handle` 添加到应作为拖动句柄的 DOM 元素上。

_注意，由于内联内容定义上就是内联元素，你的组件也应该返回一个 HTML 内联元素。_
...
然后你可以使用此自定义 schema 实例化你的编辑器，具体方法请参见 [自定义 Schema](/docs/features/custom-schemas) 页面。