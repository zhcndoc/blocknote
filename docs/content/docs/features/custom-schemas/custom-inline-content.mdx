...
`content:` 定义内联内容包含哪种可编辑内容：

- `"styled"` 用于富文本，即 [`StyledText`](/docs/foundations/document-structure#inline-content-objects)。
- `"plain"` 用于未设置样式的纯文本，例如内联代码或内联数学源代码。纯文本内容不能包含格式标记（粗体、斜体等），并且它是唯一支持[语法高亮](/docs/features/blocks/code-blocks#syntax-highlighting)的内容类型。
- `"none"` 表示内联内容不包含可编辑内容。

<Callout type="info">
  在提及演示中，我们希望每个提及都是一个单一的、不可编辑的元素，因此我们将
  `content` 设置为 `"none"`。
</Callout>
...
- `contentRef:` 一个 React `ref`，您可以使用它来标记内联内容中可编辑的元素。只有当您的内联内容配置包含 `content: "styled"` 或 `content: "plain"` 时，此属性才可用。
...
<Callout type="info">
  ...
  根节点中渲染和序列化，这意味着你无法使用依赖于 React 上下文的 hooks。
</Callout>
...
<Callout type="info">
  请注意，由于内联内容本质上是内联的，你的组件也应该返回一个 HTML 内联元素。
</Callout>
