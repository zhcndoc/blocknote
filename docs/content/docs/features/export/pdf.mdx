...
`PDFExporter` 构造函数接受两个参数：`schema` 和 `mappings`。
一个 _mapping_ 定义了如何将 BlockNote schema 元素（区块、内联内容或样式）转换为 React-PDF 元素。
如果您在编辑器中使用了[自定义 schema](/docs/features/custom-schemas)，或者想覆盖默认 BlockNote 元素的 PDF 转换方式，您可以传入自定义的 `mappings`：
...
[数学](/docs/features/blocks/math)和[图表](/docs/features/blocks/diagrams)区块自带 PDF 映射——数学区块会导出为矢量公式，内联数学公式会导出为随文本流动的图像，图表会导出为嵌入式图像。有关设置，请参阅[导出数学公式](/docs/features/blocks/math#pdf)和[导出图表](/docs/features/blocks/diagrams#exporting)。
