...
斜杠菜单是一种建议菜单，使用 `/` 字符打开（或者点击[区块侧边菜单](/docs/react/components/side-menu)中的 `+` 按钮时打开）。
...
具有相同 `group` 属性的项必须在数组中**相邻**，才能作为一组渲染。如果相同 `group` 的项被不同 `group` 的项隔开，它们会被渲染成两个独立的组，并且各自拥有自己的标签：
...
通常，建议菜单旨在通过单个字符触发。不过，也支持更长的触发字符串，例如 `img:` 或 `ref:`。

<Example name="custom-schema/suggestion-menus-mentions" />

更改新建议菜单中的项，或用于渲染它的组件，与更改[斜杠菜单](/docs/react/components/suggestion-menus#slash-menu)的方法相同。有关提及元素如何工作的更多信息，请参见[自定义内联内容](/docs/features/custom-schemas/custom-inline-content)。
