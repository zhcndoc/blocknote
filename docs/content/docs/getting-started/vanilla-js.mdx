...
每个 UI 元素都由一个[扩展](/docs/features/extensions)支持。你可以通过 `editor.getExtension(...)` 从编辑器中获取扩展实例，每个实例都会公开一个保存其当前状态的 store（可见性、位置以及任何元素特定的数据）。store 是一个小型的可观察容器，包含以下主要成员：用于读取当前值的 `state`、用于更新状态的 `setState`，以及用于接收变更通知的 `subscribe`。可用的 UI 元素扩展如下：
