---
title: 使用 Next.js
description: 了解如何将 BlockNote 集成到 Next.js 中
---

# 使用 Next.js 入门

BlockNote 是一个仅在客户端渲染（而非服务器端渲染）运行的组件。如果你使用 Next.js，需要确保 Next.js 不会尝试将 BlockNote 作为服务器端组件进行渲染。
...
这样应该可以解决在 Next.js React 应用中嵌入 BlockNote 时可能遇到的任何问题！
